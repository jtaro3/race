...
function update(dt,now){
 ...
 rivals.forEach((r,i)=>{r.speed=132+i*4+Math.sin(now/800+i)*7;r.distance+=r.speed*dt/trackLength;r.progress=r.distance%1});
 const ahead=rivals.filter(r=>r.distance>player.distance).length;$('#position').textContent=Math.min(6,ahead+1);$('#lap').textContent=player.lap;$('#speed').textContent=String(Math.round(player.speed)).padStart(3,'0');$('#raceTime').textContent=format(now-startAt);$('#bestLap').textContent=player.best?format(player.best):'--:--.---';
}
